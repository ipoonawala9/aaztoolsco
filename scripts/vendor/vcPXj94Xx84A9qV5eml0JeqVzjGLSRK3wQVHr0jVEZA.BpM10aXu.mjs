import { t as e } from "./rolldown-runtime.Dh6celcD.mjs";
import { n as t, t as n } from "./shared-lib.DAh5fv1m.mjs";
function r(e, n) {
  return {
    breakpoints: [
      { hash: `2l3yge`, mediaQuery: `(min-width: 1200px)` },
      { hash: `s1phfx`, mediaQuery: `(min-width: 810px) and (max-width: 1199.98px)` },
      { hash: `11y81x8`, mediaQuery: `(max-width: 809.98px)` },
    ],
    description: t(e, n).description,
    elements: { IikD8IlBv: `classes`, m2YVqE6PE: `classes-1`, VdncUIAEg: `classes-2` },
    robots: `max-image-preview:large`,
    serializationId: `framer-2OpoY`,
    title: t(e, n).title || `Cable Ties`,
    viewport: `width=device-width`,
  };
}
var i,
  a,
  o = e(() => {
    (n(),
      (i = 1),
      (a = {
        exports: {
          default: { type: `function`, annotations: { framerContractVersion: `1` } },
          metadataVersion: { type: `variable`, annotations: { framerContractVersion: `1` } },
          __FramerMetadata__: { type: `variable` },
        },
      }));
  });
o();
export { a as __FramerMetadata__, r as default, i as metadataVersion, o as t };
//# sourceMappingURL=vcPXj94Xx84A9qV5eml0JeqVzjGLSRK3wQVHr0jVEZA.BpM10aXu.mjs.map
