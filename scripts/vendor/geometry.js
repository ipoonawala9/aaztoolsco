google.maps.__gjsload__("geometry", function (_) {
  var msa = function (a, b) {
      return Math.abs(_.Cl(b - a, -180, 180));
    },
    nsa = function (a, b, c, d, e) {
      if (!d) {
        c = msa(a.lng(), c) / msa(a.lng(), b.lng());
        if (!e)
          return (
            (e = Math.sin(_.Pk(a.lat()))),
            (e = Math.log((1 + e) / (1 - e)) / 2),
            (b = Math.sin(_.Pk(b.lat()))),
            _.Qk(
              2 * Math.atan(Math.exp(e + c * (Math.log((1 + b) / (1 - b)) / 2 - e))) - Math.PI / 2
            )
          );
        a = e.fromLatLngToPoint(a);
        b = e.fromLatLngToPoint(b);
        return e.fromPointToLatLng(new _.Dn(a.x + c * (b.x - a.x), a.y + c * (b.y - a.y))).lat();
      }
      e = _.Pk(a.lat());
      a = _.Pk(a.lng());
      d = _.Pk(b.lat());
    ...
    psa = function (a, b, c) {
      ...
      for (let q = 0, u = g.getLength(); q < u; ++q) {
        ...
        for (let L = 0; L < B; ++L) {
          var h = x.getAt(L);
          let O = x.getAt((L + 1) % z);
          if (osa(h, a) || osa(O, a)) return !0;
          var k = _.Cl(h.lng(), -180, 180),
            m = _.Cl(O.lng(), -180, 180);
          let U = Math.max(k, m),
            A = Math.min(k, m);
          if (
            (k =
              Math.abs(_.Cl(k - m, -180, 180)) <= 1e-9 &&
              (Math.abs(_.Cl(k - d, -180, 180)) <= c || Math.abs(_.Cl(m - d, -180, 180)) <= c))
          ) {
            k = a.lat();
          ...
          if (U - A > 180 ? d + c >= U || d - c <= A : d + c >= A && d - c <= U)
            if (((h = nsa(h, O, d, f, b)), Math.abs(h - a.lat()) < c)) return !0;
        }
      }
      return !1;
    },
    ...
    usa = function (a, b) {
      ...
      for (let x = 0; x < a.length; ++x)
        (ssa(a[x], d), sB(f, d, c), tsa(c, e, e), ([f[0], f[1], f[2]] = d));
      var [g, h, k] = f,
        [m, p, q, u] = e;
    ...
    sB = function (a, b, c) {
      ...
      e = m * m + p * p + f * f + d * d;
      if (e !== 0)
        ((b = Math.sqrt(e)), (c[0] = m / b), (c[1] = p / b), (c[2] = f / b), (c[3] = d / b));
      else {
    ...
    tB = class {};
  tB.isLocationOnEdge = psa;
  tB.containsLocation = function (a, b) {
    a = _.vm(a);
    var c = _.Cl(a.lng(), -180, 180),
      d = !!b.get("geodesic"),
      e = b.get("latLngs"),
      f = b.get("map");
    ...
    for (let k = 0, m = e.getLength(); k < m; ++k) {
      ...
      for (let q = 0, u = p.getLength(); q < u; ++q) {
        let x = p.getAt(q),
          z = p.getAt((q + 1) % u);
        var h = _.Cl(x.lng(), -180, 180);
        let B = _.Cl(z.lng(), -180, 180),
          L = Math.max(h, B);
        h = Math.min(h, B);
        (L - h > 180 ? c >= L || c < h : c < L && c >= h) &&
          nsa(x, z, c, d, f) < a.lat() &&
          (g = !g);
      }
    }
  ...
  var uB = class {};
  uB.computeSignedArea = usa;
  uB.computeArea = function (a, b) {
    if (!(a instanceof _.fo || Array.isArray(a) || a instanceof _.sn || a instanceof _.mo))
      try {
        a = _.un(a);
      } catch (c) {
        try {
          a = new _.mo((0, _.Qja)(a));
        } catch (d) {
          throw _.Vl("Invalid path passed to computeArea(): " + JSON.stringify(a));
        }
      }
    b = b || 6378137;
    if (a instanceof _.mo) {
      if (a.getRadius() === void 0)
        throw _.Vl("Invalid path passed to computeArea(): Circle is missing radius.");
      if (a.getRadius() < 0)
        throw _.Vl("Invalid path passed to computeArea(): Circle must have non-negative radius.");
      if (b < 0)
        throw _.Vl(
    ...
    if (a instanceof _.sn) {
      ...
      let c = 2 * Math.PI * b ** 2 * (1 - Math.cos(((a.lat.lo - 90) * Math.PI) / 180));
      c -= 2 * Math.PI * b ** 2 * (1 - Math.cos(((a.lat.hi - 90) * Math.PI) / 180));
      return (c * Math.abs(a.lng.hi - a.lng.lo)) / 360;
    }
    return Math.abs(usa(a, b));
  };
  uB.computeLength = function (a, b) {
    b = b || 6378137;
    var c = 0;
    a instanceof _.fo && (a = a.getArray());
    for (let d = 0, e = a.length - 1; d < e; ++d) c += rsa(a[d], a[d + 1], b);
    return c;
  };
  uB.computeDistanceBetween = rsa;
  uB.traversePath = function () {
    throw Error("traversePath() is not available in this version of the Maps JavaScript API.");
  };
  uB.interpolate = function (a, b, c) {
    a = _.vm(a);
    b = _.vm(b);
    var d = _.sm(a),
      e = _.tm(a),
    ...
    b = qsa(a, b);
    var m = Math.sin(b);
    if (m < 1e-6) return new _.rm(a.lat(), a.lng());
    a = Math.sin((1 - c) * b) / m;
    c = Math.sin(c * b) / m;
    b = a * h * Math.cos(e) + c * k * Math.cos(g);
    e = a * h * Math.sin(e) + c * k * Math.sin(g);
    return new _.rm(
      _.Qk(Math.atan2(a * Math.sin(d) + c * Math.sin(f), Math.sqrt(b * b + e * e))),
      _.Qk(Math.atan2(e, b))
    );
  };
  uB.computeOffsetOrigin = function (a, b, c, d) {
    a = _.vm(a);
    c = _.Pk(c);
    b /= d || 6378137;
    d = Math.cos(b);
    ...
    a = _.tm(a) - Math.atan2(b, d * Math.cos(g) - e * Math.sin(g));
    return new _.rm(_.Qk(g), _.Qk(a));
  };
  uB.computeOffset = function (a, b, c, d) {
    a = _.vm(a);
    b /= d || 6378137;
    c = _.Pk(c);
    var e = _.sm(a);
    ...
    return new _.rm(_.Qk(Math.asin(g)), _.Qk(a + Math.atan2(b * e * Math.sin(c), d - f * g)));
  };
  uB.computeHeading = function (a, b) {
    a = _.vm(a);
    b = _.vm(b);
    var c = _.sm(a),
      d = _.tm(a);
  ...
  var wsa = { encoding: _.rt, spherical: uB, poly: tB };
  _.Va.google.maps.geometry = wsa;
  _.dl("geometry", wsa);
});
